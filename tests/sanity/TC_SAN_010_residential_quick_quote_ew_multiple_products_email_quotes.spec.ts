...
test.describe('@sanity | E2E | Residential Quick Quote | England & Wales', () => {
  test('TC_SAN_010 | Residential quick quote E&W (multiple products) + email quotes + close dialog', async ({ page }) => {
    ...
    if (await searchAddress.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const preferred = page
        .getByRole('option', { name: /Nexus.*Leadenhall|Leadenhall.*EC3A|EC3A.*Leadenhall/i })
        .first();
