...
// Purpose: Validate broker portal manual address entry with maximum 255 character address
...
test.describe('@sanity | E2E | Broker Portal | Commercial | Address Validation', () => {
  test('TC_SAN_018 | Create Commercial Northern Ireland policy with manual 255 character address entry', async ({ page }) => {
    test.setTimeout(120000);
    const caseRef = `E2E-COMM-NI-255ADDR-${Date.now()}`;
    ...
    // 4) Select 2 products and click Proceed. Verify Step 2 Statements of Fact loads.
    ...
    // 7) Enter insured name, postcode, and manual 255 character address line 1, town/city and click Proceed.
    // Verify Step 5 Summary loads.
    await finalDetails.fillRequiredDetailsWithLongAddress();
    await finalDetails.proceed();
    await summary.expectLoaded();

    // 8) Verify summary data: case ref, limit £500,000.00, insured name, long address (truncated or full).
