...
test.describe('@regression | E2E | Cancellation | Cancel and Reissue | Scotland', () => {
  test('TC_REG_015_SCOT_DOCS | Cancel and reissue a live policy (Scotland) with issue docs assertion', async ({ page }) => {
    test.setTimeout(900000);
    test.slow();

    const caseRef = `E2E-CAN-REISSUE-SCOT-${Date.now()}`;
    ...
    // Local Scotland start flow (kept in this test to avoid changing shared page objects).
    const startScotlandQuote = page.getByRole('link', { name: /Scotland Start quote/i }).first();
    await expect(startScotlandQuote).toBeVisible({ timeout: 120000 });
    await startScotlandQuote.click();
