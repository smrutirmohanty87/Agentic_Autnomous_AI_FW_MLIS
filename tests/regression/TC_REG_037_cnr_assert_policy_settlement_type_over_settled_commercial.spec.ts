...
test.describe('@regression | E2E | Commercial | Cancel and Reissue | Policy Settlement Type', () => {
  test('TC_REG_037 | Commercial CnR then assert Policy Settlement Type is Over Settled on Details tab', async ({ page }) => {
    test.setTimeout(900000);
    test.slow();

    const caseRef = `E2E-CNR-COMM-OVER-SETTLEMENT-${Date.now()}`;
    ...
    // Run Cancel and Reissue flow with Settlement Type override.
    ...
    await salesforce.completeCancelAndReissueDialog({
      ...
      settlementType: 'Over Settled',
      description: `Commercial cancel and reissue over-settled assertion (${policyNumber})`,
    });
    ...
    // Reopen policy and validate Details tab field.
    ...
    await expect(settlementFieldContainer).toContainText(/Over Settled/i);
  });
});
