/**
 * orchestrator/live-demo-mta-002.ts
 *
 * New live MTA demo flow with generator-timed testcase materialization.
...
const SUITE = 'MTA Live Demo Workflow 002';
const GENERATED_TEST_FILE = resolve(__dirname, '../tests/sanity/TC_SAN_MTA_002_live_generated.spec.ts');
...
async function main(): Promise<void> {
  ...
  if (existsSync(GENERATED_TEST_FILE)) {
    ...
    console.log(`[live-demo-mta-002] Removed pre-existing testcase before run: ${GENERATED_TEST_FILE}`);
  }
  ...
  const testCases: TestCase[] = [
    {
      id: 'TC_MTA_002',
      name: 'Create base policy and initiate MTA flow (live demo 002)',
      suite: SUITE,
      url: BASE_URL,
      steps: [
        {
        ...
        {
          description: 'Navigate to quote manager and start MTA base policy 002',
          action: async (page) => {
            const loginPage = new CommercialLoginPage(page);
            const quoteManager = new CommercialQuoteManagerPage(page);
            ...
            await productSelection.fillCaseReferenceAndLimit(`MTA2-BASE-${Date.now()}`, '500000');
            await productSelection.selectProductsByIndex([1]);

            if (!recoveryGate.ready) {
              throw new Error('Intentional MTA 002 live-demo failure to trigger Healing and RCA');
            }
          },
        },
      ],
  ...
  const result = await orchestrate({
    ...
    postExecutionSelfHealRetry: {
      ...
      onBeforeRetry: () => {
        ...
        console.log('[live-demo-mta-002] Recovery gate opened for retry run.');
      },
    },
  });
...
main().catch(err => {
  console.error('[live-demo-mta-002] Unhandled error:', err);
  process.exit(1);
});
