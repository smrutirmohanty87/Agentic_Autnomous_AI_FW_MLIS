/**
 * orchestrator/live-demo-commercial-7p.ts
 *
 * Demo entry point for the Commercial England & Wales policy flow
 * using exactly 7 products while showing every agent live on the dashboard.
 *
 * Run:
 *   npx ts-node orchestrator/live-demo-commercial-7p.ts
 */
...
const SUITE = 'Commercial England & Wales 7-Product Live Demo';
const GENERATED_TEST_FILE = resolve(__dirname, '../tests/sanity/TC_SAN_001_create_commercial_ew_policy_seven_products.spec.ts');
...
function writeGeneratedSevenProductSpec(): void {
...
test.describe('@sanity | E2E | Commercial | England & Wales', () => {
  test('TC_SAN_001 | Create Commercial England & Wales policy (exactly 7 products)', async ({ page }) => {
    test.setTimeout(120000);
    const caseRef = \`E2E-COMM-7P-\${Date.now()}\`;
    ...
    await productSelection.selectProductsByIndex([1, 2, 3, 4, 5, 6, 7]);
...
`;
  ...
  console.log(`[commercial-live-demo-7p] Generated testcase at Generator phase: ${GENERATED_TEST_FILE}`);
}
...
async function main(): Promise<void> {
  ...
  if (existsSync(GENERATED_TEST_FILE)) {
    ...
    console.log(`[commercial-live-demo-7p] Removed pre-existing testcase before run: ${GENERATED_TEST_FILE}`);
  }
  ...
  const testCases: TestCase[] = [
    {
      id: 'TC_COMM_7P_001',
      name: 'Create Commercial England & Wales policy with exactly 7 products',
      suite: SUITE,
      url: BASE_URL,
      steps: [
        {
        ...
        {
          description: 'Select exactly 7 products and complete the quote flow',
          action: async (page) => {
            const quoteManager = new CommercialQuoteManagerPage(page);
            const productSelection = new CommercialProductSelectionPage(page);
            const statements = new CommercialStatementsOfFactPage(page);
  ...
  const result = await orchestrate({
    ...
    demoMode: true,
    onAfterGenerator: () => {
      writeGeneratedSevenProductSpec();
    },
    postExecutionSelfHealRetry: {
      ...
      onBeforeRetry: () => {
        ...
        console.log('[commercial-live-demo-7p] Recovery gate opened for the retry run.');
      },
    },
  });
...
main().catch(err => {
  console.error('[commercial-live-demo-7p] Unhandled error:', err);
  process.exit(1);
});
