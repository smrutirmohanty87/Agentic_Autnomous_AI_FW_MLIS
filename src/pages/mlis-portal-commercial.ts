...
export class CommercialLoginPage {
  ...
  async login(email: string, password: string) {
    await smartFill(this.page, 'MLIS Email address', email, { context: 'CommercialLogin' });
    await smartFill(this.page, 'MLIS Password', password, { context: 'CommercialLogin' });
    await smartClick(this.page, 'MLIS Login', { context: 'CommercialLogin' });
  }
}
...
export class CommercialQuoteManagerPage {
  ...
  async startCommercialEnglandWalesQuote() {
    await this.acceptCookiesIfVisible();
    await smartClick(this.page, 'MLIS Commercial England & Wales Start quote', {
      context: 'CommercialQuoteManager',
      nth: 1,
...
export class CommercialProductSelectionPage {
  ...
  async fillCaseReferenceAndLimit(caseRef: string, limit: string) {
    await smartFill(this.page, 'MLIS Commercial Case reference', caseRef, { context: 'CommercialProductSelection' });
    await this.page.keyboard.press('Tab').catch(() => undefined);

    await smartFill(this.page, 'MLIS Commercial Limit of indemnity', limit, { context: 'CommercialProductSelection' });
    await this.page.keyboard.press('Tab').catch(() => undefined);
  ...
  async selectProductsByIndex(indexes: number[]) {
    ...
    for (let i = 0; i < indexes.length; i += 1) {
      await smartClick(this.page, 'MLIS Commercial Product Select', {
        context: 'CommercialProductSelection',
        nth: indexes[i],
        firstMatchOnly: false,
      });
      if (i === 0) {
  ...
  async proceed() {
    await smartClick(this.page, 'MLIS Commercial Proceed', { context: 'CommercialProductSelection' });
  }
}
...
export class CommercialStatementsOfFactPage {
  ...
  async confirmAllStatements() {
    ...
    while (remaining > 0 && safety < 50) {
      ...
      await smartClick(this.page, 'MLIS Commercial Statements Confirm', { context: 'CommercialStatements', firstMatchOnly: true });
  ...
  async proceed() {
    await smartClick(this.page, 'MLIS Commercial Proceed', { context: 'CommercialStatements' });
  }
...
export class CommercialQuotesPage {
  ...
  async expectLoaded() {
    ...
    // "Quote summary" is sometimes rendered disabled while quote cards are still initializing.
    // The stable ready signal is that at least one "Select quote" button is enabled.
  ...
  async selectFirstQuote() {
    await smartClick(this.page, 'MLIS Commercial Select quote', { context: 'CommercialQuotes' });
  }
}
...
export class CommercialFinalPolicyDetailsPage {
  ...
  async fillRequiredDetails() {
    ...
    await smartFill(this.page, 'MLIS Commercial Final details required input', 'E2E Test Client', {
      context: 'CommercialFinalDetails',
      nth: 0,
      firstMatchOnly: false,
    });
    ...
    if (await enterManually.isVisible({ timeout: 2000 }).catch(() => false)) {
      await smartClick(this.page, 'MLIS Commercial Enter manually', { context: 'CommercialFinalDetails' });
    }
  ...
  async proceed() {
    await smartClick(this.page, 'MLIS Commercial Proceed', { context: 'CommercialFinalDetails' });
  }
}
...
export class CommercialSummaryPage {
  ...
  async proceedToOrder() {
    await smartClick(this.page, 'MLIS Commercial Proceed to order', { context: 'CommercialSummary' });
  }
}
...
export class CommercialOrderDialog {
  ...
  async selectTodayAndOrder() {
    await smartClick(this.page, 'MLIS Commercial Confirm policy commencement', { context: 'CommercialOrderDialog' });
    await smartClick(this.page, 'MLIS Commercial Today', { context: 'CommercialOrderDialog' });
    await expect(this.page.getByRole('button', { name: 'Order now' })).toBeEnabled();
    await smartClick(this.page, 'MLIS Commercial Order now', { context: 'CommercialOrderDialog' });
  }
}
...
export class CommercialPolicyIssuedPage {
  ...
  async backToQuoteManager() {
    await smartClick(this.page, 'MLIS Commercial Back to quote manager', { context: 'CommercialPolicyIssued' });
  }
}
