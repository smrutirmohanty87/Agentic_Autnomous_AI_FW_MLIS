...
export class NiLoginPage {
  ...
  async login(email: string, password: string) {
    await smartFill(this.page, 'MLIS Email address', email, { context: 'NiLogin' });
    await smartFill(this.page, 'MLIS Password', password, { context: 'NiLogin' });
    await smartClick(this.page, 'MLIS Login', { context: 'NiLogin' });
  }
}
...
export class NiQuoteManagerPage {
  ...
  async startResidentialNorthernIrelandQuote() {
    await this.acceptCookiesIfVisible();
    await smartClick(this.page, 'MLIS Northern Ireland Start quote', { context: 'NiQuoteManager' });
...
export class NiProductSelectionPage {
  ...
  async fillCaseReferenceAndLimit(caseRef: string, limit: string) {
    await smartFill(this.page, 'MLIS Commercial Case reference', caseRef, { context: 'NiProductSelection' });
    await this.page.keyboard.press('Tab').catch(() => undefined);

    await smartFill(this.page, 'MLIS Commercial Limit of indemnity', limit, { context: 'NiProductSelection' });
    await this.page.keyboard.press('Tab').catch(() => undefined);
  ...
  async proceed() {
    await smartClick(this.page, 'MLIS Commercial Proceed', { context: 'NiProductSelection' });
  }
}
...
export class NiStatementsOfFactPage {
  ...
  async confirmAllStatements() {
    ...
    while (remaining > 0 && safety < 50) {
      ...
      await smartClick(this.page, 'MLIS Commercial Statements Confirm', { context: 'NiStatements' });
  ...
  async proceed() {
    await smartClick(this.page, 'MLIS Commercial Proceed', { context: 'NiStatements' });
  }
}
...
export class NiQuotesPage {
  ...
  async selectFirstQuote() {
    await smartClick(this.page, 'MLIS Commercial Select quote', { context: 'NiQuotes' });
  }
}
...
export class NiFinalPolicyDetailsPage {
  ...
  async fillRequiredDetails() {
    ...
    if (await enterManually.isVisible({ timeout: 2000 }).catch(() => false)) {
      await smartClick(this.page, 'MLIS Commercial Enter manually', { context: 'NiFinalDetails' });
    }
  ...
  async proceed() {
    await smartClick(this.page, 'MLIS Commercial Proceed', { context: 'NiFinalDetails' });
  }
}
...
export class NiSummaryPage {
  ...
  async proceedToOrder() {
    await smartClick(this.page, 'MLIS Commercial Proceed to order', { context: 'NiSummary' });
  }
}
...
export class NiOrderDialog {
  ...
  async selectTodayAndOrder() {
    await smartClick(this.page, 'MLIS Commercial Confirm policy commencement', { context: 'NiOrderDialog' });
    await smartClick(this.page, 'MLIS Commercial Today', { context: 'NiOrderDialog' });
    await expect(this.page.getByRole('button', { name: 'Order now' })).toBeEnabled();
    await smartClick(this.page, 'MLIS Commercial Order now', { context: 'NiOrderDialog' });
  }
}
...
export class NiPolicyIssuedPage {
  ...
  async backToQuoteManager() {
    await smartClick(this.page, 'MLIS Commercial Back to quote manager', { context: 'NiPolicyIssued' });
  }
}
