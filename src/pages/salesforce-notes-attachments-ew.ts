...
export class SalesforceNotesAttachmentsEwPage {
  ...
  async searchPolicyAndOpenFromGlobalSearchGrid(policyReference: string) {
    ...
    // Use a deterministic target (header vs dialog) so we don't fill the wrong input.
    if (activeSearchInput === headerSearchInput) {
  ...
  async openNotesAndAttachmentsFromRelatedTab() {
    ...
    await smartClick(this.page, 'SF Related tab', { context: 'SalesforceNotesAttachments' });
    await this.waitForLightningIdle();
    ...
    await smartClick(this.page, 'SF Notes & Attachments link', { context: 'SalesforceNotesAttachments' });
    await this.waitForLightningIdle();
