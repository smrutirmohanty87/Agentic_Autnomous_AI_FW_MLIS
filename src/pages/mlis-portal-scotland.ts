...
export class ScotlandLoginPage {
  ...
  async login(email: string, password: string) {
    await smartFill(this.page, 'MLIS Email address', email, { context: 'ScotlandLogin' });
    await smartFill(this.page, 'MLIS Password', password, { context: 'ScotlandLogin' });
    await smartClick(this.page, 'MLIS Login', { context: 'ScotlandLogin' });
  }
}
...
export class ScotlandQuoteManagerPage {
  ...
  async startResidentialScotlandQuote() {
    await this.acceptCookiesIfVisible();
    await smartClick(this.page, 'MLIS Scotland Start quote', { context: 'ScotlandQuoteManager' });
...
export class ScotlandProductSelectionPage {
  ...
  async fillCaseReferenceAndLimit(caseRef: string, limit: string) {
    await smartFill(this.page, 'MLIS Commercial Case reference', caseRef, { context: 'ScotlandProductSelection' });
    await this.page.keyboard.press('Tab').catch(() => undefined);

    await smartFill(this.page, 'MLIS Commercial Limit of indemnity', limit, { context: 'ScotlandProductSelection' });
    await this.page.keyboard.press('Tab').catch(() => undefined);
  ...
  async selectProductsByIndex(indexes: number[]) {
    ...
    for (let i = 0; i < indexes.length; i += 1) {
      await smartClick(this.page, 'MLIS Commercial Product Select', {
        context: 'ScotlandProductSelection',
        nth: indexes[i],
  ...
  async proceed() {
    await smartClick(this.page, 'MLIS Commercial Proceed', { context: 'ScotlandProductSelection' });
  }
}
...
export class ScotlandStatementsOfFactPage {
  ...
  async confirmAllStatements() {
    ...
    while (remaining > 0 && safety < 50) {
      ...
      await smartClick(this.page, 'MLIS Commercial Statements Confirm', { context: 'ScotlandStatements' });
  ...
  async proceed() {
    await smartClick(this.page, 'MLIS Commercial Proceed', { context: 'ScotlandStatements' });
  }
}
...
export class ScotlandQuotesPage {
  ...
  async selectFirstQuote() {
    await smartClick(this.page, 'MLIS Commercial Select quote', { context: 'ScotlandQuotes' });
  }
}
...
export class ScotlandFinalPolicyDetailsPage {
  ...
  async fillRequiredDetails() {
    ...
    if (await enterManually.isVisible({ timeout: 2000 }).catch(() => false)) {
      await smartClick(this.page, 'MLIS Commercial Enter manually', { context: 'ScotlandFinalDetails' });
    }
  ...
  async proceed() {
    await smartClick(this.page, 'MLIS Commercial Proceed', { context: 'ScotlandFinalDetails' });
  }
}
...
export class ScotlandSummaryPage {
  ...
  async proceedToOrder() {
    await smartClick(this.page, 'MLIS Commercial Proceed to order', { context: 'ScotlandSummary' });
  }
}
...
export class ScotlandOrderDialog {
  ...
  async selectTodayAndOrder() {
    await smartClick(this.page, 'MLIS Commercial Confirm policy commencement', { context: 'ScotlandOrderDialog' });
    await smartClick(this.page, 'MLIS Commercial Today', { context: 'ScotlandOrderDialog' });
    await expect(this.page.getByRole('button', { name: 'Order now' })).toBeEnabled();
    await smartClick(this.page, 'MLIS Commercial Order now', { context: 'ScotlandOrderDialog' });
  }
}
...
export class ScotlandPolicyIssuedPage {
  ...
  async backToQuoteManager() {
    await smartClick(this.page, 'MLIS Commercial Back to quote manager', { context: 'ScotlandPolicyIssued' });
  }
}
