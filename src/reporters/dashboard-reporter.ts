...
type LiveSnapshot = {
  ...
  createdAtIso: string;
  createdAtLabel: string;
  status: FullResult['status'] | 'running';
  totalTests: number;
  startedTests: number;
...
function countTestsInSuite(suite: Suite): number {
  if (typeof (suite as any).allTests === 'function') {
    return (suite as any).allTests().length;
  }

  let count = 0;
  if (Array.isArray((suite as any).tests)) {
    count += (suite as any).tests.length;
  }
  if (Array.isArray((suite as any).suites)) {
    for (const child of (suite as any).suites) {
...
class DashboardReporter implements Reporter {
  ...
  onBegin(config: FullConfig, suite: Suite) {
    ...
    this.totalTests = countTestsInSuite(suite);
    this.testsStarted = 0;
    this.runningTests = 0;
    this.passedCount = 0;
    this.failedCount = 0;
    ...
    this.liveSnapshotPath = path.join(this.outputDir, 'runs', `run-${this.runId}.progress.json`);

    this.writeLiveSnapshot('running');
    this.openDashboard();
  }
  ...
  onTestBegin(test: TestCase) {
    ...
    const id = `${browserProject}:${file}:${test.title}`;
  ...
  onTestEnd(test: TestCase, result: TestResult) {
    ...
    if (this.runningTests === 0) {
      this.currentTest = null;
    }

    if (result.status === 'passed') this.passedCount += 1;
    else if (result.status === 'failed') this.failedCount += 1;
    else if (result.status === 'skipped') this.skippedCount += 1;
    else if (result.status === 'timedOut') this.timedOutCount += 1;
    else if (result.status === 'interrupted') this.interruptedCount += 1;
  ...
  private writeLiveSnapshot(status: FullResult['status'] | 'running') {
    const snapshot: LiveSnapshot = {
      ...
      startedTests: this.testsStarted,
      completedTests: this.testEntries.length,
      passed: this.passedCount,
      failed: this.failedCount,
      skipped: this.skippedCount,
      timedOut: this.timedOutCount,
      interrupted: this.interruptedCount,
      running: this.runningTests,
      pending: Math.max(0, this.totalTests - this.testsStarted),
      progressPct: this.totalTests > 0 ? Math.floor((this.testEntries.length / this.totalTests) * 100) : 0,
      currentTest: this.currentTest ?? undefined,
      tests: this.testEntries,
    };

    ensureDir(path.dirname(this.liveSnapshotPath));
    fs.writeFileSync(this.liveSnapshotPath, JSON.stringify(snapshot, null, 2), 'utf-8');
  }

  private async isServerReachable(serverUrl: string, timeoutMs: number = 2000): Promise<boolean> {
  ...
  private openDashboard() {
    if (!this.openAfterRun) return;

    // Generate initial dashboard HTML with historical runs (if any)
    ...
    // Phase 3: Check for dashboard server
    const serverUrl = process.env.DASHBOARD_SERVER_URL ?? process.env.DASHBOARD_URL ?? 'http://localhost:3000';
    
    const launchDashboard = async () => {
      let urlToOpen = fileUrl;
      
      // Try server first, fall back to file if unavailable
      try {
        const isReachable = await this.isServerReachable(serverUrl, 2000);
        if (isReachable) {
          urlToOpen = serverUrl;
          console.log(`[dashboard] Server reachable at ${serverUrl}`);
        } else {
          console.log(`[dashboard] Server unavailable, using local file`);
        }
      } catch {
        console.log(`[dashboard] Server check failed, using local file`);
      }
      
      this.launchBrowser(platform, urlToOpen, indexPath);
    };
    
    launchDashboard().catch((err) => {
      console.log(`[dashboard] WARNING: Dashboard launch error - ${err.message}`);
      this.launchBrowser(platform, fileUrl, indexPath);
    });
  }

  private launchBrowser(platform: string, url: string, indexPath: string) {
  ...
  async onEnd(result: FullResult) {
    ...
    this.writeLiveSnapshot(result.status);
